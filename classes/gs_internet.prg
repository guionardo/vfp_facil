...
*
* Classe de comunicação Internet
*
...
DEFINE CLASS Internet AS CUSTOM
	...
	FTPUser = 'tbyteupdate@tbyte.com.br'
	FTPPass = 'T1m3@8463'
	FTPServerPath = ''
	FTPBinary = .T.
	FTPOpen = 0
...
*
	FUNCTION ProxySave(lcArq)
		...
		IF FILE(m.lcArq) AND DeleteFile(m.lcArq)=0
			THIS.MSG("Não foi possível excluir o arquivo de configuração do proxy em "+m.lcArq,.T.)
			RETURN .F.
		ENDIF
...
*
	FUNCTION ProxyTest
		IF EMPTY(THIS.PROXYHOST) OR ;
			...
			THIS.MSG('Configurações de proxy estão inválidas!',.T.)
			RETURN .F.
		ENDIF
		...
		IF !THIS.HTTPGet(PROXY_TEST_URL,@lcRetorno,.F.,.F.,'Teste de proxy')
			...
			IF !THIS.HTTPGet(PROXY_TEST_URL,@lcRetorno,.F.,.F.,'Teste de proxy desativado')
				...
				RETURN .F.
			ELSE
				THIS.MSG('Conexão OK sem necessidade de proxy!')
				RETURN .T.
			ENDIF
		ENDIF
...
*
	...
	FUNCTION FTPConnect(strHost, strUser, strPwd)
		...
		IF THIS.FTPSession = 0
			...
			= InternetCloseHandle (THIS.FTPOpen)
			THIS.FTPOpen = 0
			THIS.MSG("Erro de conexão: "+m.lcErro,.T.)
			RETURN .F.
		ENDIF
		WAIT CLEAR
