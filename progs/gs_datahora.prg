...
FUNCTION DataHoraInternet
	...
	IF !FILE(m.lcArq)
		MESSAGEBOX("W32TM.EXE não encontrado!")
		RETURN CTOT("")
	ENDIF
	...
	IF m.lnE<=32
		MESSAGEBOX("Erro na execução de W32TM.EXE"+CHR(13)+;
	...
	IF !FILE(m.lcTmp)
		...
		ENDDO
		IF !FILE(m.lcTmp)
			MESSAGEBOX("Arquivo "+m.lcTmp+" não encontrado!",48,"NTP data/hora ("+m.lcNTPServer+")",5000)
			RETURN CTOT("")
		ENDIF
	ENDIF
	...
	DO WHILE FILE(m.lcTmp)	AND (m.lnT>0)
		IF DeleteFile(m.lcTmp)=0
			IF FILE(m.lcTmp)
				WAIT WINDOW "Limpando temporários..." TIMEOUT 1
			ENDIF
		ENDIF
		m.lnT = m.lnT - 1
	ENDDO
	...
	m.lnSec = VAL(m.lcSec)
	m.lcDT= CTOT(ALLTRIM(STREXTRACT(m.lcDT,"‚",".")))+VAL(M.lcSec)
